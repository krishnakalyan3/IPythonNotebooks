...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    AvgRating=float(sum(IDandRatingsTuple[1]))/float(len(IDandRatingsTuple[1]))
    
    return (MovieID,(NoRatings,AvgRating))
...
movieNameWithAvgRatingsRDD = moviesRDD.join(movieIDsWithAvgRatingsRDD).map(lambda x : x[1]).map(lambda x : (x[1][1],x[0],x[1][0]))
