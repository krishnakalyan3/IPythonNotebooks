...
def tokenize(string):
    ...
    """
    new_list = []
    for w in [x for x in re.split(split_regex, string.lower()) if x]:
...
#print googleRecToToken.take(10)
...
# TODO: Replace <FILL IN> with appropriate code
#print amazonRecToToken.map(lambda x : (x[0],x[1])).takeOrdered(1, key=lambda x: -len(x[1]))
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return amazonRecToToken.map(lambda x : (x[0],x[1])).takeOrdered(1, key=lambda x: -len(x[1]))
...
def idfs(corpus):
    ...
    return (tokenSumPairTuple.map(lambda a : (a[0],(N/a[1]))))
...
crossSmall = (googleSmall
              ...
              .cache())
#print googleSmall.cartesian(amazonSmall).map(lambda a : a[1]).map(lambda x : x[0]).take(5)
...
#     googleRec = record[0]
#     amazonRec = record[1]
#     googleURL = crossSmall.map(lambda a : a[0]).map(lambda x : x[0])
#     amazonID = crossSmall.map(lambda a : a[1]).map(lambda x : x[0])
#     googleValue = crossSmall.map(lambda a : a[0]).map(lambda x : x[1])
#     amazonValue = crossSmall.map(lambda a : a[1]).map(lambda x : x[1])
...
avgSimDups = trueDupsRDD.map(lambda x : x[1]).map(lambda x : x[0]).sum()/trueDupsCount


nonDupsRDD = (sims.subtractByKey((goldStandard)))


avgSimNon = nonDupsRDD.map(lambda x : x[1]).sum()/nonDupsRDD.count()
...
a1= amazonNorms.collectAsMap()
amazonNormsBroadcast = sc.broadcast(a1)
...
amazonInvPairsRDD = (amazonWeightsRDD
                    .map(lambda x : invert(x)).flatMap(lambda x : x)
                    .cache())
