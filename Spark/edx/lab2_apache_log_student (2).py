...
not200 = access_logs.filter(lambda access_logs : access_logs.response_code != 200)
...
#print dayToHostPairTuple.take(2)

dayGroupedHosts = dayToHostPairTuple.groupByKey()
#print dayGroupedHosts.take(2)
...
dayAndHostTupleU = access_logs.map(lambda x : (x.date_time.day,x.host)).distinct().groupByKey().mapValues(len)

dayAndHostTuple = access_logs.map(lambda x : (x.date_time.day,x.host)).groupByKey().mapValues(len)

joinx = dayAndHostTuple.join(dayAndHostTupleU)
#print joinx.collect()

avgDailyReqPerHost = joinx.map(lambda (a, (b, c)) : (a, b/c)).sortByKey(lambda x : x)
...
# TODO: Replace <FILL IN> with appropriate code
#print avgDailyReqPerHost.take(2)
...
#print badEndpointsTop20.take(21)
...
#print errHostsCountPairTuple.take(12)
...
#print errDateCountPairTuple.take(2)
errDateSum = errDateCountPairTuple.reduceByKey(lambda a,b : a + b)
#print errDateSum.take(2)
errDateSorted = errDateSum.map(lambda x : (x[1],x[0]) )
#print errDateSorted.take(2)
                 #               <FILL IN>)

errByDate = errDateSorted.sortByKey().map(lambda x: (x[1],x[0])).sortByKey().collect()
errDateSorted.cache()
#print errByDate.take(2)
...
daysWithErrors404 = errDateSorted.map(lambda x : (x[1],x[0])).map(lambda x : x[0] ).takeOrdered(27,lambda x : x)
errors404ByDay = errDateSorted.map(lambda x : (x[1],x[0])).sortByKey().map(lambda x : x[1]).take(27)
...
topErrDate = errDateSorted.map(lambda x : (x[1],x[0])).takeOrdered(5,lambda x : -x[1])
...
hoursWithErrors404 = hourRecordsSorted.map(lambda x : (x[0],x[1])).map(lambda x : x[0] ).takeOrdered(27,lambda x : x)
