...
aSparse = SparseVector(4, dict(enumerate(aDense)))

bDense = np.array([0., 0., 0., 1.])
bSparse = SparseVector(4, dict(enumerate(bDense)))
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    MyDict ={}
    for a in rawFeats:
        MyDict[OHEDict.get(a)] =1
   
    return SparseVector(numOHEFeats,MyDict)
...
def createOneHotDict(inputData):
    ...
    Returns:
        dict: A dictionary where the keys are (featureID, value) tuples and map to values that are
            unique integers.
    """
    return inputData.flatMap(lambda x : x).distinct().zipWithIndex()
...
def parsePoint(point):
    ...
    return list(enumerate(ipSplit[1:]))
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    return SparseVector(numOHEFeats,[(OHEDict[element],1) for element in rawFeats if (element in OHEDict)])
...
def evaluateResults(model, data):
    ...
    """
    prob = data.map(lambda x: (getP(x.features,model.weights,model.intercept),x.label))
    print prob.take(1)
    return prob.map(lambda x:computeLogLoss(x[0],x[1])).mean()
...
def parseHashPoint(point, numBuckets):
    ...
    Returns:
        LabeledPoint: A LabeledPoint with a label (0.0 or 1.0) and a SparseVector of hashed
            features.
    """
    feats = parsePoint(point)
    label = point.split(",")[0]
    #print newPoint.take(1)
    hashFeats = hashFunction(numBuckets, feats, printMapping=False)
    #print hashFeats
    return LabeledPoint(label,hashFeats)
