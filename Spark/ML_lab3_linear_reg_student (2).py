...
#print parsePoint('2001.0,0.884,0.610,0.600,0.474,0.247,0.357,0.344,0.33,0.600,0.425,0.60,0.419')
...
#print parsedDataInit.take(2)
onlyLabels = parsedDataInit.map(lambda x : x.label)
#print onlyLabels.take(1)
...
def calcRMSE(labelsAndPreds):
    ...
    """
    return (labelsAndPreds.map(lambda x : ((x[0]-x[1])**(2)) ).mean())**(1.0/2)
...
# TODO: Replace <FILL IN> with appropriate code
labelsAndPredsTrain = parsedTrainData.map(lambda x : x.label).mean()
rmseTrainBase = calcRMSE(parsedTrainData.map(lambda x : (x.label,labelsAndPredsTrain)))

labelsAndPredsVal = parsedValData.map(lambda x : x.label).mean()
rmseValBase = calcRMSE(parsedValData.map(lambda x : (x.label,labelsAndPredsTrain)))

labelsAndPredsTest = parsedTestData.map(lambda x : x.label).mean()
rmseTestBase = calcRMSE(parsedTestData.map(lambda x : (x.label,labelsAndPredsTrain)))
...
def gradientSummand(weights, lp):
    ...
    ex1 = weights
    ex2 = lp.features 
    return (np.dot(ex1, ex2)-lp.label)*ex2
...
def twoWayInteractions(lp):
    ...
    """
    #twoWayExample = twoWayInteractions(lp)
    
    #label= lp.label
    newf = list(tuple(lp.features) + (tuple(a*b for a, b in list(itertools.product(lp.features,repeat=2)))))
    return LabeledPoint(lp.label, newf)
...
#print labelsAndPredsInteract.take(2)
